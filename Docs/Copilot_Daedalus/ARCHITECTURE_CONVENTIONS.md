...
来源与先例：`HandState`（`CD-005`，现已演化为 `CardZoneState`，见 `CD-015`），详见 `plans/2026-07-29-battlescene-drag-to-play-minimal.md`。
...
按 `architecture.md` 的三层模型，像 `CardZoneState` 这样的纯 C# 状态类，目前对应"状态层"的**早期过渡实现**（用 `event Action` 而非 R3 `ReactiveProperty`）。这不是绕开 R3，而是状态层设计本身还没定（`STATUS.md` Open Question 里的"是否引入 VContainer/R3/UniTask"仍未答复）。一旦项目正式接入 R3，需要一次专门的迁移任务，不能在某个具体功能开发中顺手引入 R3 打破这个约定的一致性。
...
- 未来是否所有"最小状态聚合"类（`CardZoneState` 等）都要合并成同一个 `BattleState` 的子聚合，还是保持多个独立聚合并存？

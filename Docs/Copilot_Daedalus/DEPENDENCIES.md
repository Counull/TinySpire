...
| DEP-007 | Battle child Scope 已可从父 `IBattleSetupOptionsSource` 注入 hero / encounter / seed；当前生产仍因没有 Run 来源而回落到 Inspector，根种子/存档尚未实现 | 需要先实现 `RunState`、战斗创建标识、随机流派生/恢复规则，以及初始 HP / 牌组输入快照 | `TinySpire/Assets/Scripts/Battle/BattleLifetimeScope.cs`、`TinySpire/Assets/Scripts/Battle/BattleSession.cs` | `plans/2026-07-30-card-zones-deterministic-random.md`、`plans/2026-08-14-battlescene-to-run-seam-corrections.md` | open | M9F 的连续重开复用 Inspector `1001:5001:5`，只证明场内可复现重置。2026-08-14 交接 seam 修复已建立父 Scope hero / encounter / seed 注入与 Inspector fallback，但没有创建 RunState、根种子派生/恢复或 HP / 牌组快照，因此本依赖继续 open。 |

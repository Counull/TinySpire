...
| DEP-012 | M7 当时的 `Card` 配置尚无成功出牌后的权威 Discard / Exhaust 归宿字段 | 首张真实 Exhaust 卡需要稳定配置字段、生成数据、写入前冻结归宿与统一 CardZones 执行；不得按模板 ID、卡名或 EffectType 硬编码 | `DataTables/Datas/battle.card.xlsx`、`TinySpire/Assets/Scripts/Core/Generated/Config/battle/Card.cs`、`TinySpire/Assets/Scripts/Battle/Turn/BattleTurnController.cs` | `plans/2026-08-02-m7-effect-executor.md`、`plans/2026-08-06-sts2-v01071-ironclad-card-pool.md` | resolved | I4 已加入 `PlayDestination` / `UpgradedPlayDestination`，Turn 在首次权威写入前冻结 Discard / Exhaust 并经唯一 CardZones seam 执行；Tremble 真实 Exhaust 的 RED→GREEN、生成、完整 EditMode 与构建证据见 `06_testing/2026-08-06-sts2-ironclad-i4-success-destination.md`。当前代码还支持 Power 归宿。 |

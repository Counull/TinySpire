...
目录 ownership 是职责与落点，不是常驻编辑权限。讨论、评审、解释或可行性任务保持只读；只有用户在当前任务中明确要求实现、修改、整理、删除等写入动作时，Agent 才能在授权范围内修改。写入授权也不会自动包含 commit、push 或无关文件。
...
commit / push 必须得到当前任务的明确用户授权。若用户已经明确要求“完成后提交并 push”，该指令可覆盖本次已审查的任务范围，无需再次索取同一授权；若尚未授权，则 AI 必须展示提交范围并等待批准。
...
AI 必须先扫描、分类并收敛到最小范围。只有不同选择会实质改变语义、风险或授权范围时才请用户选择；明确、可回滚且位于用户点名区域内的维护可以直接实施。不得借模糊措辞扩展到无关目录或大规模重构。
